...
from app.domain.types import Score, ScoreComponent, SearchCriteria
...
class TestPonderations:
    ...
    def test_les_poids_se_modifient_sans_toucher_a_l_algorithme(self):
        ...
        autres = Weights(capacity=55, equipment=0, building=15, floor=10, occupancy=12, history=8)
        petite = room("Petite", capacity=10, equipment_ids=frozenset())
        critere = SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO}))
        assert score_room(petite, critere, weights=autres).total > score_room(
            petite, critere
        ).total
...
class TestCapacite:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(10, 12, pytest.approx(0.958, abs=0.01), id="leger_surdimensionnement_tolere"),
    ...
    def test_la_grande_salle_est_penalisee(self):
        ...
        assert score_room(room(capacity=12), critere).total > score_room(
            room(capacity=60), critere
        ).total
...
class TestEquipements:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(frozenset({VIDEO}), frozenset({VIDEO}), 1.0, id="tout_present"),
            pytest.param(
                frozenset({VIDEO, TABLEAU}), frozenset({VIDEO}), 0.5, id="moitie_presente"
            ),
...
class TestBatimentEtEtage:
    ...
    def test_batiment_habituel_a_defaut(self):
        ...
        assert building_fit(salle, SearchCriteria(), user(preferred_building_id=ANNEXE)) == 1.0
    ...
    def test_proximite_d_etage(self, etage, attendu):
        assert floor_fit(room(floor_level=etage), user(preferred_floor_level=2)) == attendu
...
class TestOccupationEtHistorique:
    ...
    def test_historique(self):
        salle = room()
        assert history_fit(salle, user(booked_room_counts={salle.id: HISTORIQUE_PLEIN})) == 1.0
        assert history_fit(salle, user(booked_room_counts={salle.id: 1})) == pytest.approx(1 / 3)
...
class TestScore:
    ...
    def test_le_total_ne_depasse_jamais_cent(self):
        parfaite = room(capacity=10, building_id=BATIMENT, floor_level=2,
                        equipment_ids=frozenset({VIDEO}), occupancy_rate=0.0)
        critere = SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO}),
    ...
    def test_details_construits_depuis_les_donnees(self):
        ...
        critere = SearchCriteria(attendees=10)
        details = {item.key: item.detail for item in score_room(salle, critere).components}
    ...
    def test_detail_de_l_etage(self, ecart, attendu):
        ...
        detail = score_room(salle, SearchCriteria(), user(preferred_floor_level=2)).get("floor")
...
class TestEligibilite:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                room(capacity=4), SearchCriteria(attendees=10),
                "capacité insuffisante", id="sous_capacite",
            ),
            pytest.param(
                room(is_available=False), SearchCriteria(),
                "salle indisponible", id="maintenance",
            ),
            pytest.param(
                room(), SearchCriteria(equipment_ids=frozenset({VIDEO})),
                "1 équipement manquant", id="un_equipement",
            ),
            pytest.param(
                room(), SearchCriteria(equipment_ids=frozenset({VIDEO, TABLEAU})),
                "2 équipements manquants", id="plusieurs_equipements",
            ),
            pytest.param(
                room(is_accessible=False), SearchCriteria(accessible_only=True),
                "accès PMR absent", id="pmr",
            ),
        ],
    )
    ...
    def test_materiel_en_preference_n_elimine_pas(self):
        critere = SearchCriteria(equipment_ids=frozenset({VIDEO}), equipment_strict=False)
        assert eligibility_issues(room(), critere) == ()


class TestJustification:
    def test_les_deux_criteres_forts_forment_la_phrase(self):
        salle = room(capacity=12, building_id=BATIMENT, equipment_ids=frozenset({VIDEO}))
        critere = SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO}),
                                 building_id=BATIMENT)
    ...
    def test_la_reserve_nomme_le_critere_quand_le_detail_ne_le_fait_pas(self):
        salle = room(capacity=60, building_id=BATIMENT, equipment_ids=frozenset({VIDEO}))
        critere = SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO}),
                                 building_id=BATIMENT)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                SearchCriteria(attendees=10), "Sans contrainte matérielle", id="sans_materiel"
            ),
            pytest.param(
            ...
            pytest.param(
                SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO, TABLEAU, ECRAN})),
    ...
    def test_phrase_du_materiel(self, critere, extrait):
        ...
        salle = room(capacity=60, occupancy_rate=0.95,
                     equipment_ids=frozenset({VIDEO, TABLEAU}))
    ...
    def test_phrase_de_l_habitude(self):
        ...
        assert "salle habituelle" in justify(score_room(salle, SearchCriteria(attendees=1), profil))

    def test_compromis_quand_aucun_critere_ne_ressort(self):
        salle = room(capacity=60, building_id=ANNEXE, occupancy_rate=0.9)
        critere = SearchCriteria(attendees=2, equipment_ids=frozenset({VIDEO}),
                                 building_id=BATIMENT)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param("salle fermée", "Indisponible : salle fermée", id="avec_empechement"),
        ],
    )
...
class TestEvaluateEtRank:
    ...
    def test_empechement_externe_rend_ineligible(self):
        ...
        evaluee = evaluate_room(salle, SearchCriteria(attendees=10), blocker="créneau pris")
    ...
    def test_les_eligibles_passent_devant_quel_que_soit_le_score(self):
        occupee = room("Occupee", capacity=12, building_id=BATIMENT,
                       equipment_ids=frozenset({VIDEO}), occupancy_rate=0.0)
        libre = room("Libre", capacity=40, building_id=ANNEXE,
                     equipment_ids=frozenset({VIDEO}), occupancy_rate=0.5)
        critere = SearchCriteria(attendees=10, equipment_ids=frozenset({VIDEO}),
                                 building_id=BATIMENT)

        classement = rank([occupee, libre], critere, blockers={occupee.id: "déjà prise"})
